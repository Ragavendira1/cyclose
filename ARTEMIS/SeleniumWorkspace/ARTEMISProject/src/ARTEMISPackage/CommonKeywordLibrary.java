...
public class CommonKeywordLibrary {
	...
	public static void IF() throws SQLException{
		...
		try {
			...
			if(DataSplit.length < 2) {
				...
				//=============================================================================================\\
				Class<?> cif = Class.forName("ARTEMISPackage."+ TestAttributes.AutomationTool.toUpperCase());
	...
	public static void CALLTEST() throws InvalidFormatException, IOException, XmlException, SQLException, InterruptedException{
		...
		TestAttributes.calltestteststepid = Arrays.copyOf(TestAttributes.calltestteststepid, TestAttributes.calltestteststepid.length + 1);
		...
		TestAttributes.calltestteststepid[TestAttributes.calltestcounter] = TestAttributes.Step_Id;
		...
		if (DataSplit.length == 4) {
			System.out.println(FunctionLibrary.getSubTestDetails() + " - " + "Sub script '" + DataSplit[0].trim() + "' is triggered."); //only for logs not for results
		...
		TestAttributes.Step_Id = TestAttributes.calltestteststepid[TestAttributes.calltestcounter];
				
		
		System.out.println(FunctionLibrary.getSubTestDetails() + " - " + TestAttributes.ActualResult);
		TestAttributes.Keyword = "CallTest";
		TestAttributes.TakeScreenShotFlag= false;
		
		String StepSelectionQuery = "SELECT * FROM SQS_TEST_STEPS where SQS_TS_Test_Id = '" + TestAttributes.Test_Id + "' and upper(SQS_TS_Run_Type) LIKE '%YES%' and SQS_TS_Step_Id > " + TestAttributes.Step_Id + " and SQS_TS_Step_Id not in (Select SQS_TW_Step_Id from SQS_TEST_WORKFLOW where SQS_TW_Test_Id = '" + TestAttributes.Test_Id + "' and SQS_TW_Workflow_Name = '" + TestAttributes.Workflow_Name + "' and upper(SQS_TW_Test_Data) = '" + TestAttributes.DataNotApplicableString + "') order by SQS_TS_Step_Id";
		FunctionLibrary.TestStepsResults = null;
		Statement stmt= null;
		FunctionLibrary.TestStepsResults = stmt.executeQuery(StepSelectionQuery);
	}
	...
	public static void CALLFUNCTION() {
		try {
			...
			if (DataSplit.length!=0) {				
				...
				Class<?> ccf = Class.forName("ARTEMISPackage."+ TestAttributes.AutomationTool.toUpperCase());
